...
"""
OCR PDF Processing Module — JSON-only Pipeline

Usage:
    python ocr_sale_kingkong_pipeline.py --folder raw_data/sale_invoice --api-key YOUR_API_KEY
...
def convert_date_round_dd_mm_yyyy(date_str):
    """Convert DD.MM.YYYY or Thai Buddhist year to YYYY-MM-DD. Otherwise return original."""
    try:
...
# ==============================
# Non-Table Parsing (return dict only)
# ==============================
...
# ==============================
# Table Parsing (return DataFrame)
# ==============================
def parse_tables_to_df(ocr_output: str, pdf_basename: str):
    ...
    for idx, table in enumerate(tables, start=1):
        ...
        if rows and len(headers) != len(rows[0]):
            print(f"[WARN] {pdf_basename} | Table {idx} normalized mismatch still exists")
        ...
        # safety: drop any row that still contains total wording
        ...
        # drop obviously empty row (all blanks)
        df = df[~(df.astype(str).apply(lambda r: (r.str.strip() == "").all(), axis=1))]
    ...
    # replace underscores with dots in headers (table columns only)
...
def dataframe_to_enriched_rows(df: pd.DataFrame, metadata: dict):
    ...
    for _, row in df.iterrows():
        ...
        for col, val in row.items():
            ...
            # monetary fields: cast to float with 2 decimals
...
def main():
    ...
    parser.add_argument("--output-dir", default="processed_data/sale_invoice", help="Where to save JSON outputs.")
